...
class ShowResult extends Component {
  ...
  showCover = () => {
    const { show_cover } = this.state;
    this.setState({show_cover:!show_cover})
  }
  ...
  render() {
    const { score, score_tip, show_cover } = this.state
    return (
      <div className="result_body">
        ...
        <div className={classnames({'share_cover':true,'hide_cover':!show_cover})} onClick={()=>this.showCover()}>
          <img src={require('../images/5-2.png')} className="share_img"></img>
        </div>
      </div>
    )
  }
